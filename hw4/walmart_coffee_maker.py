...
def test_that_toasters_appear_in_search_at_correct_price_and_rating(browser):
	...
	# Walmart appears in the first page of results
	browser.implicitly_wait(4)
	results = browser.find_element_by_id("ires")
	targets = results.find_elements_by_tag_name("a")
	link = None
	for target in targets:
		if target.get_attribute("href").startswith("http://www.walmart.com"):
			link = target.get_attribute("href")
	assert link != None
		
	# Customer clicks the Walmart link
	browser.get(link)
	browser.implicitly_wait(4)
	assert "Walmart" in browser.title	
